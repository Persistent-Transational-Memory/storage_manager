...
namespace versionm {
	...
	template<class VersionHeader>
	struct PMEMNUMAVersionManagerBasic {
		using VersionHeaderType = VersionHeader;

		using VersionAllocator = allocator::SimpleNUMALocalPmemAllocator;
	};

	template<class VersionHeader>
	class PMEMNUMAVersionManager : public SimpleVersionManagerTemplate<
			typename PMEMNUMAVersionManagerBasic<VersionHeader>::VersionHeaderType,
			typename PMEMNUMAVersionManagerBasic<VersionHeader>::VersionAllocator
	>{
	public:
		using BaseType = SimpleVersionManagerTemplate<
				typename PMEMNUMAVersionManagerBasic<VersionHeader>::VersionHeaderType,
	...
	public:
		PMEMNUMAVersionManager(size_t tuple_size, size_t expected_amount): BaseType(tuple_size, expected_amount) {}
	};
